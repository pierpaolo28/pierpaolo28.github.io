...
![](/../assets/img/0_N.png)<br>Figure 1: 0-1 vs 1-N (Image by author).
...
![](/../assets/img/hypotesis_testing.png)<br>Figure 3: Different stages of prototyping (Image by author).
...
$$CAC = \dfrac{marketing \times sales\:expenses}{\#\:of\:new\:users}$$
...
$$ \text{Average Customer Lifetime} = \dfrac{1}{churn\:rate} $$

At this point we just need to calculate the average profit per user per year in order to calculate the LTV:

$$ \text{Average Profit per User per Year} = \dfrac{average\:revenue\:per\:user\:per\:year\:(ARPU)}{gross\:margin\:\%} $$

Putting the 2 together we can then calculate the LTV:

$$ LTV = \dfrac{(Annual\:ARPU \times gross\:margin\:\%)}{churn\:rate} $$
